...
export const updateBrowserAction = async (action, tmp = false) => {
  if (!tmp) self.currentBrowserAction = action
  /* eslint-disable-next-line */
  if (!self.coverBrowserAction) self.coverBrowserAction = () => {}
  ...
  /* eslint-disable-next-line */
  self.coverBrowserAction = () => {}
  if (action === 'popup') {
    await browser.action.setPopup({popup: 'index.html#/popup'})
  } else {
    await browser.action.setPopup({popup: ''})
    self.browswerActionClickedHandler = getBrowserActionHandler(action)
    if (!self.opts.openTabListWhenNewTab) return
    self.coverBrowserAction = async activeInfo => {
      const tab = await browser.tabs.get(activeInfo.tabId)
      if (['about:home', 'about:newtab', 'chrome://newtab/'].includes(tab.url)) {
        return updateBrowserAction('show-list', true)
      } else {
        return updateBrowserAction(self.currentBrowserAction)
      }
    }
  }
}
